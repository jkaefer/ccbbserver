...
from regsnp_intron.protein_feature import  fetch_db
from regsnp_intron.protein_feature.spined import SpineD
from regsnp_intron.protein_feature.ptm import PTM
from regsnp_intron.protein_feature.pfam import Pfam
from regsnp_intron.protein_feature.gene_pred_ext import GenePredExt
    ...
    def calculate_protein_feature(self, bfname, out_fname):
        with open(os.path.expanduser(bfname)) as bed_f, open(os.path.expanduser(out_fname), 'w') as out_f:
            ...
            for line in bed_f:
                ...
                out_f.write('\t'.join(list(map(str, cols + list(disorder) + list(ss) + list(asa) + [ptm, pfam]))) + '\n')

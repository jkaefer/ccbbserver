    ...
    def extract_transcripts(self):
        ...
        for query_bin, transcript_id, chrom, strand, txStart, txEnd, cdsStart, cdsEnd, exonCount,\
            ...
            if cdsStartStat != 'none' or cdsEndStat != 'none':  # if it's a protein coding gene.
                txStart, txEnd, cdsStart, cdsEnd, exonCount = list(map(int, [txStart, txEnd, cdsStart, cdsEnd, exonCount]))
                exonStarts = list(map(int, exonStarts.rstrip(',').split(',')))
                exonEnds = list(map(int, exonEnds.rstrip(',').split(',')))
                exonFrames = list(map(int, exonFrames.rstrip(',').split(',')))
    ...
    def get_protein_coord(self, transcript_id, gstart, gend):
        ...
        if transcript_id in self.transcripts:
            ...
            if gstart < cdsEnd and gend > cdsStart:
                ...
                if strand == '-':
                    exonsExt=list(exons)
                    revExons=list(reversed(list(exonsExt)))
                    for estart, eend in revExons:
                        if cdsEnd < estart:
                            continue
                        cstart = estart
                        cend = eend

    ...
    def parse_matrix(self, fname):
        ...
        with open(fname) as f:
            ...
            for line in f:
                ...
                matrix.append(list(map(float, cols[1:])))
    ...
    def cal_junction_strength(self, ref_fname, bed_fname, out_fname):
        with open(bed_fname) as bed_f, open(out_fname, 'w') as out_f:
            ...
            for line in bed_f:
                ...
                start_snp, end_snp, start, end, distance = list(map(int, [start_snp, end_snp, start, end, distance]))
                if strand == '+':
                ...
                out_f.write('\t'.join(list(map(str, cols + [astart, aend, aseq, dstart, dend, dseq,
                                                       aic, dic, aic_change, dic_change])))+ '\n')
            ref.close()
    
    def reverse_complement(self, seq):
        if seq:
            trans_tab = str.maketrans('ACGTacgt', 'TGCAtgca')

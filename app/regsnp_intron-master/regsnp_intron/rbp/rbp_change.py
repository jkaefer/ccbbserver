...
from regsnp_intron.rbp import rbp_score
    ...
    def parse_mean_var(self, fname):
        ...
        with open(os.path.expanduser(fname)) as f:
            ...
            for line in f:
                ...
                mean_var[motif] = list((map(float, [bmean, bsd, nbmean, nbsd])))
        return mean_var
    ...
    def cal_change(self, score_fname, out_fname, keep_ncol=6):
        ...
        with open(os.path.expanduser(score_fname)) as score_f, open(os.path.expanduser(out_fname), 'w') as out_f:
            ...
            for line in score_f:
                ...
                ref_scores = list(map(float, cols[keep_ncol:(keep_ncol + self.rbps.pssm_num)]))
                alt_scores = list(map(float, cols[(keep_ncol + self.rbps.pssm_num):]))
                ...
                out_f.write('\t'.join(list(map(str, [chrom, pos, ref, alt, ref_seq, alt_seq] + log_ors + pvalues))) + '\n')

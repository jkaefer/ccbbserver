...
my $job = $ARGV[1];
my $motiflist = "/N/slate/jkaefer/splicingdb/pwm.all.list";
my @pwmrows = ("A","C","G","T");
...
while(<IN>)
{
   ...
   $pwm{'len'} = $mo_len;
   close PW;
   $flname =~ s/\/N\/slate\/jkaefer\/splicingdb\/motif\/PWM\///;
   print $flname;
   $motifs{$flname} = {%pwm};
   %pwm = ();
...
my @pwm_out_ord = ();
open(IN, "/N/slate/jkaefer/splicingdb/pwm_output_order")||die "$!";
while(<IN>){
s/\s+$//;
push @pwm_out_ord, $_;
}
...
while(<IN>)
{
	...
	while($mask =~ /[^\x00]+/g)
	{
		...
		foreach  my $pp (@pwm_out_ord)
		{
		    ...
		    $m_pp =~ s/\/N\/slate\/jkaefer\/splicingdb\/motif\/PWM\///;
		    #print $m_pp;

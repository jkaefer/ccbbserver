...
a<- system('pwd',intern=TRUE);
write.csv(a, file = "my_data.csv");
...
ratio.filename = "/N/slate/jkaefer/splicingdb/pwm.all.list";
...
job = commandArgs(TRUE)[2];
#fprefix = "hgmd.vcf";

sfl = paste(job,"/", fprefix, ".motif_change.out", sep='' );
write.csv(sfl, file = "my_data.csv");
tg.bind.score = read.table(sfl,quote="", sep="\t", header=F, skip=1);
#pwm.list = as.vector(unlist(read.table("/data/pro/indels/motif/pwm")))
pwm.list = as.vector(unlist(read.table("/N/slate/jkaefer/splicingdb/pwm.all")));
...
sfl = paste(job, "/", fprefix, ".logor", sep='' )
write.table(file = sfl,indel.logor.tg.yy , quote=F, row.names =F , sep = "\t")

sfl = paste(job, "/", fprefix, ".changeBinding.pvalue", sep='' )
